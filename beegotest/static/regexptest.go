// 字符串和正则表达式中都存在一些特殊字符，例如 \
// 正则表达式的实现不同，特殊字符也不同
// 例如，re2正则表达式中的特殊字符：* . ? + $ ^ < > ( ) [ ] { } | \  等
// 特殊字符单独出现时，表示一种意思；多个特殊字符结合出现时，表示另一种意思
// 举例：\ 将下一个字符标记为特殊字符、或原义字符、或向后引用、或八进制转义符
//        \n 将原义字符n标记为特殊字符换行符，\* 将特殊字符*标记为原义字符*
//      ^ 匹配开始位置; [^abc] 在方括号表达式中使用时，表示不接受该字符集合
package main
